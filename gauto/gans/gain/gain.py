...
def gain(data_x, gain_parameters, ori_data_x):
    ...
    for it in tqdm(range(iterations)):
        ...
        # Sample hint vectors
        H_mb_temp = binary_sampler_by_columns(
            data_x[:batch_size], np.sqrt(dim), hint_rate
        )
        H_mb = M_mb * H_mb_temp
        ...
        _, D_loss_curr, d_merge = sess.run(
            [D_solver, D_loss_temp, d_sum_merge], feed_dict={M: M_mb, X: X_mb, H: H_mb}
        )
        _, G_loss_curr, MSE_loss_curr, g_merge = sess.run(
            [G_solver, G_loss_temp, MSE_loss, g_sum_merge],
        ...
        if it in range(0, iterations, 20):
            plot_genarated_versus_real(
                ori_data_x,
                imputed_data,
                15,
                "output/run_test_it_" + str(it) + ".png",
                MSE_loss_curr,
                it,
                data_m,
            )
