...
class AutoEncoderDecoder:
    def __init__(
        ...
        self.percentage_val = (
            perc_val  # percentage of validation. overwritten if data_validation exists
        )
        # variables
        self.encoder = []
        self.decoder = []
    ...
    def __encoder_decoder(self):
        ...
        for i in range(self.nb_layers):
            ...
            encoder = layers.MaxPooling2D(
                (self.pooling[i], self.pooling[i]), padding="same"
            )(encoder)
    ...
    def train(self, training_data, target_data, validation_data=None):
        ...
        # encoder
        self.encoder = Model(
            self.autoencoder.layers[1].input, self.autoencoder.layers[1].output
        )
        # decoder
        self.decoder = Model(
            self.autoencoder.layers[2].input, self.autoencoder.layers[2].output
        )
